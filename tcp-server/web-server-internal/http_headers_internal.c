...
bool headers_add(HttpHeaders *headers, const char* key, const char* value) {
  assert(headers && "headers_add: headers is null, did you forgot to call create_http_headers()?");
  ...
  if(headers->length == headers->capacity) {
    KeyValuePair *temp;
    temp = realloc(headers->headers, sizeof(KeyValuePair) * (headers->capacity + 1));
    if(temp == NULL) {
      return false;
    }
